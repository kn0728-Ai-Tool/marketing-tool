    ...
    for r in filtered:
        ...
        for i, ad in enumerate(r.get("ad_copies", [])[:3]):
            with ad_cols[i]:
                ...
                appeal_html = (
                    f'<div class="ad-appeal">✨ {appeal_point}</div>'
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("v2.3 | Powered by OpenAI")
...
with tab_chart:
    ...
    g1, g2 = st.columns(2)
    with g1:
        st.plotly_chart(make_segment_pie_chart(seg_counts), use_container_width=True)
    with g2:
        st.plotly_chart(make_intent_bar_chart(valid), use_container_width=True)
...
with tab_guide:
    ...
    st.markdown("""
### STEP 1　キーワードを入力する
1行1つ・最大20件まで分析できます。メモ・業種を入力すると分析精度が上がります。
 
---
 
### STEP 2　分析開始ボタンを押す
AIがキーワードごとに以下を自動生成します。結果はDBに自動保存されます。
...
| 購買意欲スコア | 1〜10点で評価 |
| 広告文案 | 価格帯ルールに沿った3パターン＋訴求ポイント |
| 感情分析 | ユーザーの検索時の感情 |
| 差別化ポイント | 競合との差別化案 |
| CTA案 | 最適なボタンテキスト |
| LP改善提案 | ランディングページの改善点 |
...
「🗄️ 分析履歴」タブで過去の分析を再表示・推移グラフを確認できます。
